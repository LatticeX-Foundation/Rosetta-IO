...
template<typename... Args>
inline void audit(string_view_t fmt, const Args &... args)
{
    default_logger_raw()->audit(fmt, args...);
}
...
template<typename T>
inline void audit(const T &msg)
{
    default_logger_raw()->audit(msg);
}
...
template<typename... Args>
inline void audit(wstring_view_t fmt, const Args &... args)
{
    default_logger_raw()->audit(fmt, args...);
}
...
#if SPDLOG_ACTIVE_LEVEL <= SPDLOG_LEVEL_AUDIT
#define SPDLOG_LOGGER_AUDIT(logger, ...) SPDLOG_LOGGER_CALL(logger, spdlog::level::audit, __VA_ARGS__)
#define SPDLOG_AUDIT(...) SPDLOG_LOGGER_AUDIT(spdlog::default_logger_raw(), __VA_ARGS__)
#else
#define SPDLOG_LOGGER_AUDIT(logger, ...) (void)0
#define SPDLOG_AUDIT(...) (void)0
#endif
