...
int gmtassl_init_ssl_ctx(
  ...
  if (*ctx == NULL) {
    ...
    log_error << "Init, SSL_CTX_new failed." ;
  ...
  if (!(SSL_CTX_set_mode(*ctx, SSL_MODE_AUTO_RETRY) & SSL_MODE_AUTO_RETRY)) {
    ERR_print_errors_fp(stdout);
    log_error << "Init, SSL_CTX_set_mode failed." ;
  ...
  if (SSL_CTX_load_verify_locations(*ctx, root_.c_str(), NULL) <= 0) {
    ERR_print_errors_fp(stdout);
    log_error << "Init, SSL_CTX_load_verify_locations failed." ;
...

  {
    ...
    if (SSL_CTX_use_certificate_file(*ctx, cert_.c_str(), SSL_FILETYPE_PEM) <= 0) {
      ERR_print_errors_fp(stdout);
      log_error << "Init, SSL_CTX_use_certificate_file failed." ;
    ...
    if (SSL_CTX_use_PrivateKey_file(*ctx, prik_.c_str(), SSL_FILETYPE_PEM) <= 0) {
      ERR_print_errors_fp(stdout);
      log_error << "Init, SSL_CTX_use_PrivateKey_file failed." ;
    ...
    if (!SSL_CTX_check_private_key(*ctx)) {
      ERR_print_errors_fp(stdout);
      log_error << "Init, SSL_CTX_check_private_key failed." ;
...
#if USE_GMTASSL
  {
    ...
    if (SSL_CTX_use_enc_certificate_file(*ctx, enc_cert_.c_str(), SSL_FILETYPE_PEM) <= 0) {
      ERR_print_errors_fp(stdout);
      log_error << "Init, SSL_CTX_use_enc_certificate_file failed." ;
    ...
    if (SSL_CTX_use_enc_PrivateKey_file(*ctx, enc_prik_.c_str(), SSL_FILETYPE_PEM) <= 0) {
      ERR_print_errors_fp(stdout);
      log_error << "Init, SSL_CTX_use_enc_PrivateKey_file failed." ;
    ...
    if (!SSL_CTX_check_enc_private_key(*ctx)) {
      ERR_print_errors_fp(stdout);
      log_error << "Init, SSL_CTX_check_enc_private_key failed." ;
  ...
  if (!is_server) {
    if (!SSL_CTX_set_cipher_list(*ctx, "ECC-SM4-SM3")) {
      log_error << "Init, SSL_CTX_set_cipher_list failed." ;
...
bool show_certs(SSL* ssl, const std::string& client_ip) {
  ...
  if (client_ip != "") {
    log_info << "whitelist check [only for server], client ip:" << client_ip ;

    X509* cert = SSL_get_peer_certificate(ssl);
    if (cert != nullptr) {
      if (!ssl_valid_check((void*)cert, nullptr, client_ip)) {
        log_error << "whitelist check failed." ;
        X509_free(cert);
        return false;
      } else {
        log_info << "whitelist check success." ;
      }
      X509_free(cert);
    } else {
      log_error << "no peer cert info." ;
      return false;
    }
  }
  return true;
